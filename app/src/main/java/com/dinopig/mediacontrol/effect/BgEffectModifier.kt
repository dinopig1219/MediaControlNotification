...
internal fun Modifier.bgEffectDraw(
    ...
    painter, preset, deviceType, isDarkTheme, surface, effectBackground, isFullSize, playing, colorStage, alpha,
)
...
private class BgEffectNode(
    ...
    override fun onAttach() { if (playing) startAnimation() }
    override fun onDetach() { animationJob?.cancel(); animationJob = null }

    fun update(
        painter: BgEffectPainter, preset: BgEffectConfig.Config, deviceType: DeviceType, isDarkTheme: Boolean,
        surface: Color, effectBackground: Boolean, isFullSize: Boolean, playing: Boolean,
        colorStage: () -> Float, alpha: () -> Float,
    ) {
        this.painter = painter; this.preset = preset; this.deviceType = deviceType; this.isDarkTheme = isDarkTheme
        this.surface = surface; this.effectBackground = effectBackground; this.isFullSize = isFullSize
        this.colorStage = colorStage; this.alpha = alpha
        if (this.playing != playing) {
            this.playing = playing
            if (playing) startAnimation() else { animationJob?.cancel(); animationJob = null }
        }
        invalidateDraw()
    }
